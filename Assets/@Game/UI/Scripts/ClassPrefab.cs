...
public class ClassPrefab : MonoBehaviour
{
    ...
    [SerializeField] private GameObject m_NotSelectClass;       // 수업을 선택하지 않고 완료 버튼을 눌렀을 때 나오는 메세지
    [SerializeField] private Button m_ChoiceButton;
    [SerializeField] private PopUpUI m_PopUpProfessor;
    ...
    private void SelecteClassInfo()
    {
        ...
        for (int i = 0; i < m_SelecteClass.classData.Count; i++)
        {
            ...
            {
                m_ClassInfo.transform.GetChild(0).GetComponent<TextMeshProUGUI>().text = m_SelecteClass.classData.ElementAt(i).Value.m_ClassName;
                ...
                m_ClassInfo.transform.GetChild(3).GetChild(0).GetComponent<TextMeshProUGUI>().text = "System";
                m_ClassInfo.transform.GetChild(3).GetChild(1).GetComponent<TextMeshProUGUI>().text = m_SelecteClass.classData.ElementAt(i).Value.m_ClassSystemValue.ToString();
                m_ClassInfo.transform.GetChild(4).GetChild(0).GetComponent<TextMeshProUGUI>().text = "Contents";
                m_ClassInfo.transform.GetChild(4).GetChild(1).GetComponent<TextMeshProUGUI>().text = m_SelecteClass.classData.ElementAt(i).Value.m_ClassContentsValue.ToString();
                m_ClassInfo.transform.GetChild(5).GetChild(0).GetComponent<TextMeshProUGUI>().text = "Balance";
                m_ClassInfo.transform.GetChild(5).GetChild(1).GetComponent<TextMeshProUGUI>().text = m_SelecteClass.classData.ElementAt(i).Value.m_ClassBalanceValue.ToString();
    ...
    // 선택한 수업이 없으면 경고 문구를 띄워주고 아니라면 다음 단계로 넘어가게해주는 함수
    public void IsNotSelectClass()
    {
        if (m_SelecteClassDataList.Count == 0)
        {
            m_NotSelectClass.SetActive(true);
        }
        else
        {
            m_PopUpProfessor.TurnOnUI();
            SelecteProfessor.Instance.ProfessorInfo();
            ClassSchedule.Instance.SetRectPosition();
            SaveClassData();
            InitPopUpPanel();
        }
    }

    // 선택한 수업이 없습니다 문구 꺼주는 함수
    public void ClosePopUp()
    {
        m_NotSelectClass.SetActive(false);
    }

    // 수업 선택창에서 뒤로 갔다왔을 때 수업 정보를 보여주는 패널을 꺼주기 위한 함수
    public void InitPopUpPanel()
    {
        m_ClassInfo.SetActive(false);
    }
