    ...
    public static class MovementUtility
    {
        ...
        public static GameObject WithinSight(Transform transform, Vector3 positionOffset, float fieldOfViewAngle, float viewDistance, GameObject targetObject, Vector3 targetOffset, bool usePhysics2D, float angleOffset2D, out float angle, int ignoreLayerMask, bool useTargetBone, HumanBodyBones targetBone, bool drawDebugRay)
        {
            ...
            if (direction.magnitude < viewDistance && angle < fieldOfViewAngle * 0.5f)
            {
                ...
                if (hitTransform != null)
                {
                    if (IsAncestor(targetObject.transform, hitTransform))
                    {
                        ...
                        if (drawDebugRay)
                        {
                            UnityEngine.Debug.DrawLine(transform.TransformPoint(positionOffset), targetObject.transform.TransformPoint(targetOffset), Color.green);
                        }
#endif
                    ...
                    else
                    {
                        if (drawDebugRay)
                        {
                            UnityEngine.Debug.DrawLine(transform.TransformPoint(positionOffset), targetObject.transform.TransformPoint(targetOffset), Color.yellow);
                        }
#endif
                    }
                }
            ...
            else
            {
                ...
                if (drawDebugRay)
                {
                    UnityEngine.Debug.DrawLine(transform.TransformPoint(positionOffset), targetObject.transform.TransformPoint(targetOffset), angle >= fieldOfViewAngle * 0.5f ? Color.red : Color.magenta);
                }
#endif
            }
